...
public class Main_2573_빙산_시간초과2 {
	static int N, M;
	static int[][] map;
	...
	private static void bfs_change(int n, int m) {
		...
		while(!q.isEmpty()) {
			...
			if(sea!=0) {
				...
				if(after < 0) after=0;
				changed.add(new int[] {p.r, p.c, after});
			}
		}
	}
	public static void main(String[] args) throws IOException {
		...
		while(true) {
			...
			for(int c=0; c<changed.size(); c++) {
				int nr = changed.get(c)[0];
				int nc = changed.get(c)[1];
				int val = changed.get(c)[2];
				map[nr][nc]=val;
			}
